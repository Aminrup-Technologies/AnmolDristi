...
namespace AnmolDristi
{
    public partial class WeldingChecklistRpt : System.Web.UI.Page
    {
        ...
        private DataTable GetObservationsById(string meetingId)
        {
            ...
            using (SqlConnection con = new SqlConnection(conStr))
            {
                using (SqlCommand cmd = new SqlCommand("SELECT QuestionNumber,IsOk,Remarks,PhotoPath,description,CAPA_Report FROM WeldingChecklist WHERE HeaderID = @HeaderID", con))
                {
                    cmd.Parameters.AddWithValue("@HeaderID", meetingId);
                    con.Open();
                    SqlDataAdapter da = new SqlDataAdapter(cmd);
