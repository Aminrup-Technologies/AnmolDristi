...
namespace AnmolDristi
{
    public partial class FireExtinguisherChecklistRpt : System.Web.UI.Page
    {
        ...
        private DataTable GetObservationsById(string meetingId)
        {
            ...
            using (SqlConnection con = new SqlConnection(conStr))
            {
                using (SqlCommand cmd = new SqlCommand("SELECT QuestionNumber,IsOk,Remarks,PhotoPath,description,ExtinguisherType,FE_SerialNo,CalibrationDate,DueDate,CAPA_Report FROM FireExtinguisherChecklist WHERE HeaderID = @HeaderID", con))
                {
                    cmd.Parameters.AddWithValue("@HeaderID", meetingId);
                    con.Open();
                    SqlDataAdapter da = new SqlDataAdapter(cmd);
