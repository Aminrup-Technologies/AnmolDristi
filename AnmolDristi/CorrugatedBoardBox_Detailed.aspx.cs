    ...
    public partial class CorrugatedBoardBox_Detailed : System.Web.UI.Page
    {
        ...
        void getDetails(DataTable dt)
        {
            if (dt.Rows.Count > 0)
            {
                ...
                TB_NoOfPkt.Text = dt.Rows[0]["NoOfPackets"].ToString();
                ...
                TB_Remarks_BurstingStrength.Text = dt.Rows[0]["BS_Remarks"].ToString();

                TB_CompressionStrength.Text = dt.Rows[0]["Comp_Strength"].ToString();
                TB_Remarks_CompressionStrength.Text = dt.Rows[0]["Comp_Remarks"].ToString();

                TB_FlutePercent.Text = dt.Rows[0]["Flute_Percentage"].ToString();
                TB_Remarks_FlutePercent.Text = dt.Rows[0]["Flute_Remarks"].ToString();

                TB_MoisturePercent.Text = dt.Rows[0]["Moisture_Percentage"].ToString();
                TB_Remarks_MoisturePercent.Text = dt.Rows[0]["Moisture_Remarks"].ToString();
        ...
        private void LoadFormDetails(string FormID, string selectedPlantValue)
        {
            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                using (SqlCommand cmd = new SqlCommand("usp_GetFormsApprovalMatrix_PM", conn))
                {
                    ...
                    cmd.Parameters.AddWithValue("@FormName", "Corrugated_Board_Box_Report"); // Replace with actual value
        ...
        public void UpdateColumnBasedOnApproverType()
        {
            ...
            if (!string.IsNullOrEmpty(employeeCode))
            {
                ...
                if (!string.IsNullOrEmpty(approverType))
                {
                    ...
                    {
                        case "Approver1":
                            updateQuery = "UPDATE TRN_Corrugated_Board_Box_Report SET Approver1_Status = 1, Approver1_TimeStamp = @TimeStamp WHERE Approver1EmployeeCode = @Condition and Id=@Id";
                            break;
                        case "Approver2":
                            updateQuery = "UPDATE TRN_Corrugated_Board_Box_Report SET Approver2_Status = 1, Approver2_TimeStamp = @TimeStamp WHERE Approver2EmployeeCode = @Condition and Id=@Id";
                            break;
                        case "DottedLineApprover":
                            updateQuery = "UPDATE TRN_Corrugated_Board_Box_Report SET DottedApprover_Status = 1, DottedApprover_TimeStamp = @TimeStamp WHERE DottedLineApproverEmployeeCode = @Condition and Id=@Id";
                            break;
                    }
        ...
        public void RejectionBasedOnApproverType()
        {
            ...
            if (!string.IsNullOrEmpty(employeeCode))
            {
                ...
                if (!string.IsNullOrEmpty(approverType))
                {
                    ...
                    {
                        case "Approver1":
                            updateQuery = "UPDATE TRN_Corrugated_Board_Box_Report SET Approver1_Status = 0, Approver1_TimeStamp = @TimeStamp WHERE Approver1EmployeeCode = @Condition and Id=@Id";
                            break;
                        case "Approver2":
                            updateQuery = "UPDATE TRN_Corrugated_Board_Box_Report SET Approver2_Status = 0, Approver2_TimeStamp = @TimeStamp WHERE Approver2EmployeeCode = @Condition and Id=@Id";
                            break;
                        case "DottedLineApprover":
                            updateQuery = "UPDATE TRN_Corrugated_Board_Box_Report SET DottedApprover_Status = 0, DottedApprover_TimeStamp = @TimeStamp WHERE DottedLineApproverEmployeeCode = @Condition and Id=@Id";
                            break;
                    }
